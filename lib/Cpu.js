...
Cpu.prototype.run = function(callbacks) {
  this.running = true;
  while (this.running) {
    this.step();
  }
...
Cpu.prototype.step = function() {
  var opcode, source, target, instruction;

  instruction = this.memory.read(this.PC); // read from current memory address (word block #)
  ...
  target = (instruction >> 4) & parseInt('111111', 2);
  source = (instruction >> 10) & parseInt('111111', 2);
  ...
  switch(opcode) {
      ...
      if (this.isRegister(target)) {
        if (this.isRegister(source)) {
          ...
          if (this.pointsToNextWordLiteral(source)) {
            this.registers[target] = this.memory.read(++this.PC);
          } else {
        ...
        if (this.pointsToNextWordAddress(target)) {
         this.memory.write(this.memory.read(++this.PC), this.getValue(source));
        }
      }
      this.PC++;
      break;
    ...
    case this.opcodes['ADD']:
      if (this.isRegister(target)) {
        if (this.isRegister(source)) {
          console.log('' + this.registers[target] + ' ' + this.registers[source]);
          this.registers[target] = this.registers[target] + this.registers[source];
          console.log(this.registers[target]);
        } else {
          if (this.pointsToNextWordLiteral(source)) {
            this.registers[target] = this.registers[target] + this.memory.read(++this.PC);
          } else {
            if (this.pointsToNextWordAddress(source)) {
              var sourceAddress = this.memory.read(++this.PC);
              var sourceAddressValue = this.memory.read(sourceAddress);
              this.registers[target] = this.registers[target] + sourceAddressValue;
            } else { // literal value
              this.registers[target] = this.registers[target] + this.getValue(source);
            }
          }
        }
      } else {
        if (this.pointsToNextWordAddress(target)) {
          var targetAddress = this.memory.read(++this.PC);
          var targetAddressValue = this.memory.read(targetAddress);
          var value = 0;
          if (this.pointsToNextWordLiteral(source)) {
          ...
          this.memory.write(targetAddress, targetAddressValue + value);
        }
      }
      this.PC++;
      break;
