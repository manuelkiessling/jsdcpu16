...
define([], function() {
  ...
  Cpu.prototype.resolve = function(data) {
    ...
    if (this.isNextWordPlusRegister(data)) {
      resolved.address = this.memory.read(++this.registers[0x1c]) + this.registers[data - 0x10];
      resolved.value = data;
      return resolved;
    }
  ...
  Cpu.prototype.stepMustBeSkipped = function() {
    if (this.skipNext) { // IFE, IFE, IFG or IFB turned out false in the previous step
     this.skipNext = false;
     this.write(0x1c, this.registers[0x1c] + 1);
     return true;
    }
    return false;
  };
  ...
  Cpu.prototype.step = function() {
    var instruction, opcode, target, source;
    ...
    if (opcode === this.opcodes['NONBASIC']) {
      opcode = (instruction >> 4) & parseInt('111111', 2);
      source = this.resolve((instruction >> 10) & parseInt('111111', 2));

      if (this.stepMustBeSkipped()) { // We need to resolve values first in order to have PC correctly set
        return this.step();
      }
  ...
  Cpu.prototype.executeNonbasicInstruction = function(opcode, source) {
    switch (opcode) {
      case this.nbopcodes['JSR']:
        var sp = this.resolve(0x1a); // resolve PUSH in order to change SP correctly and get the address in the SP
        if (source > 31) {
          // Next word is the source value
          this.write(this.registers[0x1b], this.registers[0x1c] + 2);
        } else {
          // Next word is the next instruction
          this.write(this.registers[0x1b], this.registers[0x1c] + 1);
        }
